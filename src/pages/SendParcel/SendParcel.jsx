...
const SendParcel = () => {
  ...
  useEffect(() => {
    const handleClickOutside = (event) => {
      if (
        ...
        !senderDropdownRef.current.contains(event.target)
      ) {
        setSenderDropdownOpen(false);
      }
      if (
        receiverDropdownRef.current &&
        !receiverDropdownRef.current.contains(event.target)
      ) {
  ...
  const onSubmit = (data) => {
    const { parcelType, parcelWeight, senderWarehouse, receiverWarehouse } =
      data;
      ...
      html: `
                <div style="text-align: left; font-size: 14px;">
                    ...
                    <p style="margin-bottom: 5px;">Sender: ${
                      senderCenter
                        ? `${senderCenter.district}, ${senderCenter.city}`
                        : "N/A"
                    }</p>
                    <p style="margin-bottom: 5px;">Receiver: ${
                      receiverCenter
                        ? `${receiverCenter.district}, ${receiverCenter.city}`
                        : "N/A"
                    }</p>
                    <p style="margin-bottom: 5px;">Delivery: ${
      ...
      if (result.isConfirmed) {
        ...
        axiosSecure
          ...
          .then((res) => {
            ...
            const insertedId =
              res.data?.insertedId || res.data?._id || res.data?.id;
            ...
            // Show success message with tracking ID and database ID, then redirect to My Parcels
            Swal.fire({
              icon: "success",
              title: "Parcel Saved!",
              html: `
                                <p>Your parcel has been saved successfully to the database.</p>
                                <p style="margin-top: 10px;"><strong>Tracking ID:</strong> ${
                                  payload.trackingID
                                }</p>
                                ${
                                  insertedId
                                    ? `<p style="margin-top: 10px;"><strong>Database ID:</strong> ${insertedId}</p>`
                                    : ""
                                }
                            `,
              confirmButtonColor: "#C8E564",
      ...
      <div className="w-full max-w-7xl">
        <div className="bg-base-200 rounded-2xl p-4 sm:p-6 md:p-8 lg:p-12 shadow-xl">
          ...
          <form
            ...
            className="rounded-lg shadow-lg p-4 sm:p-6 md:p-8 bg-base-100"
          >
            {/* Hidden meta fields */}
                        ...
                        render={({ field }) => {
                          ...
                          return (
                            ...
                            >
                              ...
                              />
                              {senderDropdownOpen &&
                                senderSearchQuery.trim() && (
                                  <div
                                    className="absolute bg-white border border-gray-300 rounded-lg shadow-lg w-full mt-1 z-[1000] max-w-full"
                                    style={{
                                      maxHeight: "300px",
                                      overflowY: "auto",
                                    }}
                                  ...
                                  >
                                    <ul className="menu p-0 m-0">
                                      {filteredReceiverWarehouses.length > 0 ? (
                                        filteredReceiverWarehouses.map(
                                          (center, index) => (
                                            <li
                                              key={index}
                                              className="border-b border-gray-100 last:border-b-0"
